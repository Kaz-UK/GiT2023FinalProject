...
def add_new_review():
    ...
    if request.method == 'POST':
        ...
        game_name = form.game_list.data
        if len(stars) == 0:
            error = "Please give a review"
        else:
            review = Review(review=review, stars=stars,
                           review_date=review_date, customer_id=customer_id, game_id=game_id)
            service.add_new_review(review)
            reviews = service.get_all_reviews()
            return render_template('review.html', reviews=reviews, stars=stars, message=error, game_id=game_id)
